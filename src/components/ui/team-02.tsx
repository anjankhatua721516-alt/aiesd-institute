...
import { Linkedin, Twitter, Instagram, Globe, ExternalLink, ArrowRight } from "lucide-react";
...
export const defaultTeamData: TeamMember[] = [
  {
    name: "Martha Finley",
...
const Team: React.FC<TeamProps> = ({
  badge = "Team",
  title = "Meet our team",
  description = "Our team is committed to redefining digital experiences through innovative web solutions while fostering a diverse and collaborative environment.",
  members = defaultTeamData,
  showAction = true,
}) => {
  const displayMembers = members && members.length > 0 ? members : defaultTeamData;
      ...
      <div className="lg:py-20 sm:py-16 py-8">
        <div className="mx-auto max-w-7xl px-4 lg:px-8 xl:px-16">
          <div className="flex flex-col items-center justify-center gap-16">
            ...
            >
              <Badge variant={"outline"} className="px-3 py-1 h-auto text-sm border-border text-foreground">
                {badge}
              </Badge>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 w-full">
              {displayMembers.map((value, index) => {
                const socials = value.socials || [
                  { icon: <InstagramIcon size={16} />, link: "#", label: "Instagram" },
                  { icon: <LinkedinIcon size={16} />, link: "#", label: "LinkedIn" },
                  { icon: <Twitter size={16} className="text-foreground" />, link: "#", label: "Twitter" },
                ];
                ...
                return (
                  ...
                  >
                    <div className="relative w-full h-80 overflow-hidden rounded-[20px] bg-[#f4f4f5] border border-border">
                      <img
                        src={value.image}
                      ...
                      {value.isFounder && (
                        <div className="absolute top-3 left-3 z-10">
                          <span className="bg-[#09090b] text-white text-[11px] font-semibold uppercase tracking-wider px-3 py-1 rounded-full shadow-sm">
                            Founder &amp; Mentor
                          </span>
                        </div>
                      )}
                      <div className="absolute inset-0 bg-gray-950/50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end justify-between p-5">
                        {value.slug ? (
                          <Link
                            to={`/faculty/${value.slug}`}
                            className="text-xs font-semibold text-white bg-white/20 hover:bg-white/30 backdrop-blur-xs px-3.5 py-2 rounded-full transition-colors flex items-center gap-1.5"
                          >
                            <span>Profile</span>
                            <ArrowRight size={13} />
                          </Link>
                        ...
                        <div className="flex gap-2">
                          {socials.map((social, idx) => (
                            <a
                              ...
                              className="flex w-fit bg-background p-2.5 text-foreground rounded-full hover:opacity-80 transition-opacity duration-300 shadow-xs"
                            >
                              {social.icon}
                            </a>
                          ))}
                    ...
                    <div className="flex flex-col gap-1 w-full">
                      <div className="flex items-center justify-between">
                        ...
                        {value.experience !== undefined && (
                          <span className="text-[11px] font-medium text-muted-foreground bg-[#f4f4f5] px-2 py-0.5 rounded-full border border-border">
                            {value.experience}+ Yrs Exp
                          </span>
                        )}
                      </div>
                      ...
                      {value.slug && (
                        ...
                        >
                          <span>View qualifications &amp; bio</span>
                          <ArrowRight size={12} />
                        </Link>
                      )}
                    </div>
